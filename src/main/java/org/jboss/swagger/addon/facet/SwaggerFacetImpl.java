...
public class SwaggerFacetImpl extends AbstractFacet<Project>implements SwaggerFacet {
  ...
  public static final Coordinate JAVADOC_PLUGIN_COORDINATE = CoordinateBuilder.create().setGroupId("org.apache.maven.plugins").setArtifactId("maven-javadoc-plugin").setVersion("2.10.3");
  ...
  private void addMavenPlugin() {
    ...
    if (!pluginFacet.hasPlugin(JAVADOC_PLUGIN_COORDINATE)) {
      javadocSwaggerPlugin = MavenPluginBuilder.create().setCoordinate(JAVADOC_PLUGIN_COORDINATE);
    } else {
      javadocSwaggerPlugin = MavenPluginBuilder.create(pluginFacet.getPlugin(JAVADOC_PLUGIN_COORDINATE));
    }

    javadocSwaggerPlugin.addExecution(ExecutionBuilder.create().addGoal("javadoc").setPhase("generate-resources").setId("generate-service-docs").setConfig(ConfigurationBuilder.create().addConfigurationElement(getDocletConfig()).addConfigurationElement(getDocletArtifact()).addConfigurationElement(getOutputDir()).addConfigurationElement(ConfigurationElementBuilder.create().setName("useStandardDocletOptions").setText("false")).addConfigurationElement(getAddidionalParam())));
  ...
  public boolean isInstalled() {
    ...
    return facet.hasPlugin(JAVADOC_PLUGIN_COORDINATE) && hasSwaggerDocletExecution(facet.getPlugin(JAVADOC_PLUGIN_COORDINATE));
  }
  ...
  private ConfigurationElement getAddidionalParam() {
    ...
    value.append("-apiVersion 1").append("\n\t\t-docBasePath ").append(configDocBasePath()).append("\n\t\t-apiBasePath ").append(configuration.getApiBasePath() == null ? projectName + "/rest" : configuration.getApiBasePath())
        /**
         * we will patch swagger ui inside forge addon so no need to use the one
         * bundled with swagger-doclet
         */
        .append("\n\t\t-swaggerUiPath ${project.build.directory}/");

    return ConfigurationElementBuilder.create().setName("additionalparam").setText(value.toString() + " \n\t\t");
  }

  /**
   * configure docbase path based on outputDir. Ex: if outputdir is
   * 'src/main/webapp/rest' then docbasePath will be: 'contextPath/rest/apidocs'
   *
   * Note that it will use 'webapp' folder as root path.
   *
   * FIXME: introduce a new param in setup command to configure docBasePath (and
   * outputDir) and remove this method
   *
   * @return
   */
  ...
  /**
   *
   * directory where swagger (json) spec files will be generated Note that it
   * must be the same dir where swagger ui artifacts reside
   * 
   * @see SwaggerFacetImpl#copySwaggerUIResources()
   * @return this
   */
  private ConfigurationElement getOutputDir() {
    return ConfigurationElementBuilder.create().setName("reportOutputDirectory").setText(configuration.getDocBaseDir());
  }
